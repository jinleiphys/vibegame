...
const GAME_ART = {
  routeLiterature: "assets/game-route-literature.png",
  routeLegacy: "assets/game-route-legacy.png",
  routeBenchmark: "assets/game-route-benchmark.png",
  auto: "assets/game-auto-navigation.png",
  audit: "assets/game-internal-audit.png",
  tenure: "assets/game-tenure-review.png",
};

const ROUTE_VISUALS = {
  literature: GAME_ART.routeLiterature,
  legacy: GAME_ART.routeLegacy,
  reproduce: GAME_ART.routeBenchmark,
};
...
const REAL_LIFE_EXCUSE_LINES = [
  ["老师这周真不好意思", "我感冒了，还起了点低烧", "看 9j symbol 看得头更疼", "白天状态不太行…"],
  ["上周末打球把脚崴了", "去办公室不方便", "我在家远程看相移", "但 VPN 老断 🥲"],
  ["老师我腰闪了…", "坐久了疼", "我先躺着读 Epelbaum 那篇 review"],
];

const TASK_RECEIVE_LINES = {
  lit: [
    ["收到，我先按 N2LO/N3LO 三体力分层整理", "重点把 c_D/c_E 拟合和 cutoff dependence 讲清楚"],
    ["这活适合先搭文献树", "我会把 Machleidt、Epelbaum、Hebeler 那几条线分开"],
    ["我先不急着写段落", "先把各组 convention 对齐，不然 intro 会写成玄学"],
  ],
  code: [
    ["我先只跑 deuteron sanity check", "3S1-3D1 coupling 不对之前不碰 He4"],
    ["我会先查 LSJ channel 顺序和 k-grid", "旧脚本能跑不代表物理对"],
    ["收到，我先把 runtime potential 的读表单独写测试", "regulator factor 也一起核掉"],
  ],
  data: [
    ["我先画 Nmax/hbarOmega 的二维趋势", "不先看 plateau 就不做外推"],
    ["收到，我会把 lambda=1.8/2.0/2.2 分开拟合", "截断误差不要硬塞进一条线"],
    ["我先做 leave-one-out 外推检查", "哪个点支配结论要先找出来"],
  ],
  proof: [
    ["这个我先从 Jacobi 坐标写起", "antisymmetrizer 的 normalization 不能一句带过"],
    ["我会把 phase convention 单独列出来", "Reviewer 2 最爱抓这种号差"],
    ["收到，我先推 A=3 的简单情形", "再推广到一般 A，不然容易漏 sqrt(A!)"],
  ],
  write: [
    ["我先按 interaction、SRG、basis truncation 三段拆", "method 不能写成代码说明书"],
    ["收到，我会先补 Hamiltonian 章节骨架", "公式少一点，但 convention 要说清楚"],
    ["我先对齐现有 draft 的语气", "别让这一段像 AI 突然插进来的"],
  ],
  exp: [
    ["我先设计 deuteron/triton/He4 三层 benchmark", "每层只回答一个物理问题"],
    ["收到，我会把张量力、3N、Coulomb 分成 ablation table", "别让 reviewer 觉得我们只会堆图"],
    ["我先列哪些 observable 真能约束结论", "binding energy 之外也要看半径或相移"],
  ],
};

const ROUTE_RECEIVE_LINES = {
  literature: [
    ["我会先补文献和 convention", "这条路线慢，但后面少踩坑"],
    ["我先把关键符号表整理出来", "不然跑代码只是把错跑快一点"],
  ],
  legacy: [
    ["我可以直接跑旧代码", "但我会标出哪些输出还没 sanity check"],
    ["我先跑 existing script 出图", "同时记一份黑箱风险清单"],
  ],
  reproduce: [
    ["我先用 benchmark 把地基打稳", "deuteron 对不上就不谈主线"],
    ["我会边跑边记录 sanity check", "以后复现不会只靠记忆"],
  ],
};

const TASK_STATUS_LINES = {
  lit: {
    progress: [
      ["文献树搭起来了", "我发现三体力那几篇对 c_D 的定义不完全一样"],
      ["intro 的逻辑我先按 cutoff dependence 写", "这样 reviewer 比较容易跟上"],
      ["我把 N2LO/N3LO 和 SRG induced terms 分成两栏了", "不然读者会混在一起"],
    ],
    done: [
      ["文献综述这段补好了", "我还加了一个 convention warning，避免后面自相矛盾"],
      ["intro 草稿交了", "从 uncertainty 切入，比硬说 ab initio 更自然"],
      ["文献脉络整理完了", "c_D/c_E、cutoff、observable 三条线都接上了"],
    ],
    excuse: [
      ["这周主要卡在文献 convention", "同一个 c_E 不同组符号差一个归一化，我不敢硬写"],
      ["我读到一篇 2005 年的 PRC", "和我们说法有冲突，我想先查清楚再下笔"],
    ],
  },
  code: {
    progress: [
      ["runtime potential 能读了", "但 3S1-3D1 coupling 的列顺序还要再核一次"],
      ["我加了 deuteron 单元测试", "现在能抓到 regulator factor 少乘一次的问题"],
      ["旧代码跑起来了", "不过 k-grid interpolation 在边界点有一个 off-by-one"],
    ],
    done: [
      ["代码实现交了", "LSJ 顺序、k-grid 和 regulator factor 都单独测过"],
      ["runtime potential 修完了", "deuteron binding energy 和相移 sanity check 都过了"],
      ["旧代码这块清干净了", "我写了 README，下一次不会再靠口口相传"],
    ],
    excuse: [
      ["我现在怀疑不是 Hamiltonian 错", "是 binary 读表 endian 或 channel map 有历史包袱"],
      ["脚本能跑但结果不稳", "我先不想把一个黑箱 bug 包装成物理结论"],
    ],
    shady: [
      ["图看起来对了", "但我其实还没确认 3D1 那列是不是转置过"],
      ["旧脚本输出已经有了", "README 说这个 channel map 以前就是这么用的，应该没事吧"],
    ],
    blameshift: [
      ["这个 channel map 是组里祖传的", "我只是照着旧 notebook 调用，没有改 convention"],
      ["旧代码没有测试不是我造成的", "我现在是在考古，不是在普通 debug"],
    ],
  },
  data: {
    progress: [
      ["48 组点我先画了 heatmap", "Nmax 外推不是每个 hbarOmega 都进 asymptotic 区"],
      ["lambda=1.8/2.0/2.2 分开看了", "趋势一致，但误差条不能太乐观"],
      ["我做了 leave-one-out", "有两个点对外推斜率影响特别大"],
    ],
    done: [
      ["外推图交了", "我把不稳定区间单独标出来，没有偷删点"],
      ["数据分析完成", "误差估计按 cutoff 和 basis 两部分拆开了"],
      ["Tjon-line 风格图做好了", "结论依赖哪些点我也写进 caption 了"],
    ],
    excuse: [
      ["外推现在很敏感", "不是我慢，是数据还没进 asymptotic 区"],
      ["hbarOmega 扫描的最优点在漂", "我不想只挑最好看的那条线"],
    ],
    shady: [
      ["我把最离群的两个 Nmax 点先隐藏了", "图立刻像一篇 paper 了"],
      ["误差条如果按最后三个点算会很好看", "按全部点算就有点吓人"],
    ],
  },
  proof: {
    progress: [
      ["推导到 A=3 的 antisymmetrizer 了", "normalization 因子现在能追到每一步"],
      ["我把 Jacobi 坐标和 single-particle convention 对齐了", "差的那个相位终于找到了"],
      ["Reviewer 2 盯的地方确实危险", "sqrt(A!) 不能靠一句话糊过去"],
    ],
    done: [
      ["推导补完了", "我把 phase convention 和 normalization 分成两个 lemma"],
      ["antisymmetrizer 那段写清楚了", "Reviewer 2 再抓只能抓我们符号丑"],
      ["公式推导交了", "A=3 例子和一般 A 的写法都放进 appendix"],
    ],
    excuse: [
      ["我卡在相位约定上", "这个负号如果错了，后面所有矩阵元都像对但其实错"],
      ["推导不是算不动", "是不同 reference 的 normalization 互相打架"],
    ],
  },
  write: {
    progress: [
      ["method 章节骨架出来了", "我按 NN+3N、SRG、basis truncation 三段写"],
      ["Hamiltonian 部分补了一版", "现在还差把 notation 和图里统一"],
      ["我在改语气", "不能让这一段看起来像直接从 AI 输出贴进去"],
    ],
    done: [
      ["写作任务完成", "method 章节现在能从 Hamiltonian 顺到 observable"],
      ["Hamiltonian 章节补好了", "我把 SRG 和 truncation 的假设都明说了"],
      ["draft 这段交了", "caption、notation 和正文已经对齐"],
    ],
    excuse: [
      ["写是写了", "但现在像 technical note，不像 PRC method section"],
      ["我发现前文 notation 没统一", "继续写只会把坑埋得更深"],
    ],
  },
  exp: {
    progress: [
      ["benchmark 表设计了一半", "deuteron、triton、He4 分别回答不同问题"],
      ["我把张量力、3N、Coulomb 做成 ablation 结构", "这样 reviewer 比较难说我们只会调参"],
      ["observable 列表出来了", "只放 binding energy 说服力不够"],
    ],
    done: [
      ["benchmark 设计交了", "每一行都对应一个 sanity check，不是凑表格"],
      ["ablation table 做好了", "张量力、三体力、Coulomb 的作用分得很清楚"],
      ["实验设计完成", "这个 benchmark 能挡住大部分 reproducibility 质疑"],
    ],
    excuse: [
      ["benchmark 不能随便堆", "如果 observable 选错，表越大越像心虚"],
      ["我在想哪些量真的独立", "binding energy 和 radius 不能都当成新证据"],
    ],
  },
};
...
function qualityPieces() {
  return researchPieces().total;
}

function researchPieces() {
  const aiHigh = state.aiDone.filter(d => d.quality >= 0.68).length;
  const studentHigh = state.assignedStudent.filter(s => s.done && s.quality >= 0.55).length;
  const bonus = state.bonusSlots || 0;
  const validation = state.validationCredits || 0;
  const support = studentHigh + bonus + validation;
  // AI can draft quickly, but every paper needs human/benchmark/audit support.
  const aiCap = Math.max(1, support * 2);
  const usableAi = Math.min(aiHigh, aiCap);
  return {
    total: usableAi + studentHigh + bonus,
    aiHigh,
    usableAi,
    blockedAi: Math.max(0, aiHigh - usableAi),
    studentHigh,
    bonus,
    validation,
    support,
...
function addValidationCredits(n, reason) {
  state.validationCredits = (state.validationCredits || 0) + n;
  if (reason) pushTerm("result", `🧾 ${reason} +${n} 验证信用`);
  checkPapers();
...
function checkWeeklyGoalCompletion(opts = {}) {
  ...
  g.completed = true;
  if (g.type === "ai") { state.cash += 200; bounceEl("cashM"); }
...
function assignStudent(uid) {
  ...
  setTimeout(() => {
    pushChat("them", studentReceiveLines(t));
    playPop();
    render();
  }, 500);
  checkWeeklyGoalCompletion();
  ...
  typewriterTerm("cmd", cmdText, () => {
    ...
    function nextLine() {
      if (i < lines.length) {
        ...
        const inv = researchPieces();
        const aiNote = highQ
          ? (inv.blockedAi > 0
              ? ` · AI 草稿待验证 ${inv.usableAi}/${inv.aiHigh} ⚠️`
              : " · AI slot 已验证 🏆")
          : " · 质量 <68% 不计入论文 ⚠️";
        const resText = `✓ done in 0.${Math.floor(Math.random()*9)+2}s · quality ${qualityPct}% · $${cost}${aiNote}`;
        pushTerm("result", resText);
        playBlip();
        if (!highQ) showAdvisorQuip(`AI 这个 ${qualityPct}% 质量组不成论文——换个更贵的 mode 试试？`);
        else if (inv.blockedAi > 0) showAdvisorQuip(`AI 草稿已经堆住了：需要学生 benchmark、组会或内部审计提供验证信用。`);
        state.aiBusy = false;
        checkPapers();
        checkWeeklyGoalCompletion();
        render();
...
function showSuspiciousModal(s) {
  const shadyLine = studentLine("shady");
  const shadyText = Array.isArray(shadyLine) ? shadyLine.join("\n") : shadyLine;
  showModal(
    "🚨 可疑交付 — 要相信她吗？",
    ...
    [
      ...
      },
      {
        ...
        fn: () => {
          ...
          state.cash -= 200;
          state.suspiciousPending = state.suspiciousPending.filter(x => x !== s);
          pushChat("them", studentLine("blameshift"), "blameshift");
...
function endWeek() {
  ...
  for (const s of active) {
    ...
    if (routeMod.studyMode && r < 0.72) {
      ...
      pushChat("them", legacyLine ? onboardingLine("legacyFast") : studentExcuseLine(s), "excuse");
      state.excuseCount = (state.excuseCount || 0) + 1;
    } else if (r < 0.8) {
      gained = 1;
      pushChat("them", studentTaskLine("progress", s));
    } else {
      gained = 2;
      pushChat("them", studentTaskLine("progress", s));
    }
    ...
    if (s.progress >= needed && s.age >= routeMod.minAge) {
      ...
      if (isSuspicious) {
        ...
        pushChat("them", studentTaskLine("shady", s), "shady");
        ...
        pushChat("them", studentTaskLine("done", s));
      }
    }
  }
  ...
  if (Math.random() < 0.3) pushChat("them", studentLine("chat"));
...
function checkPapers() {
  ...
  if (total > state.papers) {
    ...
    state.papers = total;
    pushTerm("result", `📄 论文 +${delta}！4 个已验证高质量成品组装成 1 篇。（累计 ${total}/${PAPERS_GOAL}）`);
    pushChat("sys", `— 攒够 4 个已验证高质量成品，发表了 ${delta} 篇论文 🎉 —`);
...
function runInternalAudit() {
  ...
  addValidationCredits(1, "内部审计形成可复现记录");
  checkWeeklyGoalCompletion();
  render();
}
...
function holdGroupMeeting() {
  ...
  const meetingLines = active.length > 0
    ? [
        ["老师这次组会有用", "我回去按您说的先补 sanity check，不直接信图"],
        ["我把问题拆成 benchmark、convention、observable 三块了", "不再一锅乱炖"],
        ["现在知道该先查哪个量了", "之前我一直在 He4 结果上盲人摸象"],
        ["我会把组会结论写进 README", "以后自己也能复现这条 pipeline"],
      ]
    : [
        ["老师组会讲完我清楚多了", "我把 SRG / cutoff / observable 的关系画成一张图了"],
        ["我先把 benchmark checklist 做成模板", "下次接任务直接照着过"],
      ];

  if (route === "literature" && (state.studentRouteProgress || 0) < 6) {
    state.studentRouteProgress += 1;
    pushChat("them", pickFresh(meetingLines, "student:mentor", 8));
  ...
  addValidationCredits(1, active.length > 0 ? "组会复核 active pipeline" : "组会建立 benchmark checklist");
  checkWeeklyGoalCompletion();
  render();
}
...
function scoreTaskForAI(t, mode = currentAutoMode()) {
  ...
  if (researchPieces().blockedAi > 0) score -= 60;
  return score;
}
...
function getAutoPlan(mode = currentAutoMode()) {
  ...
  const inv = researchPieces();
  if (inv.blockedAi > 0 && focusLeft > 0) {
    if (activeStudent.length > 0 && (state.weekStats?.mentor || 0) < mentorLimit) {
      return {
        kind: "mentor",
        text: `已有 ${inv.blockedAi} 个 AI 草稿缺验证，先开组会复核 benchmark。`,
        action: holdGroupMeeting,
        reason: "AI 不能无限直接转论文，必须有人工/benchmark 支撑。",
      };
    }
    if (state.cash >= AUDIT_COST) {
      return {
        kind: "audit",
        text: `已有 ${inv.blockedAi} 个 AI 草稿缺验证，先做内部审计。`,
        action: runInternalAudit,
        reason: "审计会增加验证信用，让 AI 草稿能进入论文槽。",
      };
    }
    if (normalPending.length > 0 && mode !== "deadline") {
      const studentBest = [...normalPending].sort((a, b) => scoreTaskForStudent(b, mode) - scoreTaskForStudent(a, mode))[0];
      return {
        kind: "student",
        text: `AI 草稿堆住了，派小王做可验证任务：${studentBest.name}。`,
        action: () => assignStudent(studentBest.uid),
        reason: "学生高质量交付会给 AI 草稿提供验证支撑。",
      };
    }
  }
...
function eventArtFor(ev) {
  const map = {
    collab_invite: GAME_ART.tenure,
    student_offer: GAME_ART.routeBenchmark,
    arxiv_clash: GAME_ART.auto,
    reviewer2_returns: GAME_ART.audit,
    gpu_maintenance: GAME_ART.routeLegacy,
    twitter_viral: GAME_ART.auto,
    ai_promo: GAME_ART.auto,
    chair_vacation: GAME_ART.tenure,
    anon_report: GAME_ART.audit,
    invited_talk: GAME_ART.tenure,
    student_hospital: GAME_ART.routeBenchmark,
    best_paper_nom: GAME_ART.tenure,
    reproduce_crisis: GAME_ART.audit,
    shower_idea: GAME_ART.routeLiterature,
    github_fork: GAME_ART.auto,
  };
  return map[ev.id] || GAME_ART.auto;
}

function showEventModal(ev) {
  // 包一层统一兜底 render()，防止事件 option 忘记调 render 导致 UI 不同步
  const btns = ev.options.map(o => ({ label: o.label, fn: () => { try { o.fn(); } finally { render(); } } }));
  showModal(`${ev.emoji} ${ev.title}`, ev.body, false, btns, eventArtFor(ev));
}
...
• 已验证成品：${inv.total} 个${inv.blockedAi > 0 ? `\n• 未验证 AI 草稿：${inv.blockedAi} 个` : ""}${fraud > 0 ? `\n• 可疑交付（你选择相信）：${fraud} 次` : ""}

${inv.blockedAi > 0 ? "你不是 AI 用少了，而是 AI 草稿缺少 benchmark、组会和审计验证。\n下局别一路狂点 AI：让小王复现关键 benchmark，或者花行动点做内部审计。" : pending > aiN ? "你给小王派了太多活。她还在修 phase convention 你就没时间了。\n下局试试把容易机械化的矩阵元检查丢给 Claude，但别忘了验证。" : "AI 模型用得不够狠，但更重要的是验证链不够稳。\n下局解锁 Sonnet / Opus 后可以加速产出，同时用组会和审计把草稿变成论文槽。"}`;
  }
...
function weeklyGoalProgressText() {
  ...
  if (g.type === "slot") {
    ...
    return `${gained}/${g.target} 已验证成品 · ${g.reward}`;
  }
...
function visualAtlasHtml(autoPlan, risk, inProgress) {
  ...
  const items = [
    ...
    },
    {
      ...
      value: `${inProgress}/4 verified · ${state.papers}/${PAPERS_GOAL} papers`,
      meta: inv.blockedAi > 0
        ? `${inv.blockedAi} 个 AI 草稿缺验证；去组会、审计或等学生 benchmark。`
        : "4 个已验证高质量成品合成 1 篇 PRC 级别论文。",
    },
    {
...
function isRealLifeExcuse(lines) {
  const sig = lineSignature(lines);
  return REAL_LIFE_EXCUSE_LINES.some(item => lineSignature(item) === sig);
}
function pickFresh(arr, key, windowSize = 8) {
  if (!arr || arr.length === 0) return "";
  if (!state) return pick(arr);
  if (!state.recentLineKeys) state.recentLineKeys = {};
  const recent = state.recentLineKeys[key] || [];
  const fresh = arr.filter(item => !recent.includes(lineSignature(item)));
  const picked = pick(fresh.length > 0 ? fresh : arr);
  const sig = lineSignature(picked);
  state.recentLineKeys[key] = [sig, ...recent.filter(x => x !== sig)].slice(0, windowSize);
  return picked;
}
function studentLine(category, key = category) {
  return pickFresh(STUDENT_LINES[category] || [], `student:${key}`, 10);
}
function onboardingLine(category) {
  return pickFresh(ONBOARDING_LINES[category] || [], `onboarding:${category}`, 6);
}
function studentReceiveLines(task) {
  const pool = [
    ...(TASK_RECEIVE_LINES[task.id] || []),
    ...(ROUTE_RECEIVE_LINES[routeConfig()] || []),
    ...STUDENT_LINES.receive,
  ];
  return pickFresh(pool, "student:receive", 12);
}
function studentTaskLine(category, task) {
  const pool = [
    ...(TASK_STATUS_LINES[task?.id]?.[category] || []),
    ...(STUDENT_LINES[category] || []),
  ];
  return pickFresh(pool, `student:${category}`, 18);
}
function studentExcuseLine(task) {
  const academicExcuses = (STUDENT_LINES.excuse || []).filter(item => !isRealLifeExcuse(item));
  const pool = [
    ...(TASK_STATUS_LINES[task?.id]?.excuse || []),
    ...academicExcuses,
  ];
  const weeksSinceLifeEvent = state.week - (state.lastLifeEventWeek ?? -99);
  const allowLifeEvent = weeksSinceLifeEvent >= 4 && (state.lifeEventCount || 0) < 2;
  if (allowLifeEvent && Math.random() < 0.12) {
    state.lastLifeEventWeek = state.week;
    state.lifeEventCount = (state.lifeEventCount || 0) + 1;
    return pickFresh(REAL_LIFE_EXCUSE_LINES, "student:life-event", REAL_LIFE_EXCUSE_LINES.length);
  }
  return pickFresh(pool, `student:excuse:${task?.id || "generic"}`, 18);
}
...
function showModal(title, body, pass, options, art = null) {
  if (state?.autoRunActive) stopAutoRun("等待玩家处理弹窗");
  _modalQueue.push({ title, body, pass, options, art });
  _processModalQueue();
}
function _processModalQueue() {
  if (_modalOpen || _modalQueue.length === 0) return;
  _modalOpen = true;
  const { title, body, pass, options, art } = _modalQueue.shift();
  ...
  if (mArt) {
    if (art) {
      mArt.src = art;
      mArt.classList.remove("hidden");
    } else {
      mArt.removeAttribute("src");
      mArt.classList.add("hidden");
    }
  }
